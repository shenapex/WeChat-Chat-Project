...
pub fn get_pid_by_name_and_cmd_pattern(pname: &str, cmd_pattern: &str) -> Vec<u32> {
    let mut result = vec![];
    unsafe {
        for pp in process::Proclist::new().unwrap() {
            let pi = pp.get_proc_info().unwrap();
...
pub fn read_number<T: Sub + Add + Ord + Default>(pid: u32, addr: usize) -> Result<T> {
    unsafe {
...
pub fn read_string(pid: u32, addr: usize, size: usize) -> Result<String> {
    unsafe {
        let hprocess = OpenProcess(PROCESS_VM_READ | PROCESS_QUERY_INFORMATION, false, pid)?;
...
pub fn read_string_or_ptr(pid: u32, addr: usize, size: usize) -> Result<String> {
...
pub fn read_bytes(pid: u32, addr: usize, size: usize) -> Result<Vec<u8>> {
    unsafe {
        let hprocess = OpenProcess(PROCESS_VM_READ | PROCESS_QUERY_INFORMATION, false, pid)?;
...
pub fn dump_wechat_info(pid: u32, special_data_dir: Option<&PathBuf>) -> WechatInfo {
...
pub fn scan_db_files(dir: String) -> Result<Vec<PathBuf>> {
...
pub fn read_file_content(path: &PathBuf) -> Result<Vec<u8>> {
...
pub fn decrypt_db_file_v3(path: &PathBuf, pkey: &String) -> Result<Vec<u8>> {
    const IV_SIZE: usize = 16;
    const HMAC_SHA1_SIZE: usize = 20;
    const KEY_SIZE: usize = 32;
    const AES_BLOCK_SIZE: usize = 16;
...
pub fn decrypt_db_file_v4(path: &PathBuf, pkey: &String) -> Result<Vec<u8>> {
    const IV_SIZE: usize = 16;
...
pub fn convert_to_sqlcipher_rawkey(pkey: &str, path: &PathBuf, is_v4: bool) -> Result<String> {
    const KEY_SIZE: usize = 32;
...
pub fn dump_all_by_pid(wechat_info: &WechatInfo, output: &PathBuf) {
